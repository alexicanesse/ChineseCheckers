...
MoveType ChineseCheckers::elementaryMove(PositionType original_position, PositionType arrival_position){
    ...
    if (!(0<= arrival_position[0] && arrival_position[0] <= 7 && 0<= arrival_position[1] && arrival_position[1] <= 7 && 0<= original_position[0] && original_position[0] <= 7 && 0<= original_position[1] && original_position[1] <= 7)){
        return(Illegal);
    };
    ...
    if (((c-a)*(d-b) == 0) || ((d-b)/(c-a)==-1)) {// if it's in a legal direction
        for(int i = 0; i < 2; ++i){
            for(auto x : this->position_colors_players_.at(i)){
                if (x[0] == (a + c)/2 and x[1] == (b + d)/2){
                    rep = true;
                } else if ((x[0] - a)*(d-b) == (x[1] - b)*(c - a) && (x[0] -a)*(c-a) + (x[1]-b)*(d-b) >= 0 && (x[0] -c)*(a-c) + (x[1]-d)*(b-d) >= 0){
                    return Illegal;
                };
...
bool ChineseCheckers::move(Player player,
    ...
    if(this->grid_.at(list_moves.at(0).at(0)).at(list_moves.at(0).at(1)) != player + 1)
        return false;
    ...
    if (fst_move == Illegal) return false;
        ...
        for (int i=1; i < n - 1; ++i)
            if (elementaryMove(list_moves.at(i), list_moves.at(i+1)) != Jump) return false;
    }

    
    /* Now that we know that the move is legal, we just have to apply it */
    this->grid_.at(list_moves.at(n-1).at(0)).at(list_moves.at(n-1).at(1)) = grid_.at(list_moves.at(0).at(0)).at(list_moves.at(0).at(1));
    grid_.at(list_moves.at(0).at(0)).at(list_moves.at(0).at(1)) = Empty;
    
    for(int i = 0; i < 8; ++i){
        if((this->position_colors_players_.at(player).at(i).at(0) == list_moves.at(0).at(0)) && (this->position_colors_players_.at(player).at(i).at(1) == list_moves.at(0).at(1))){
            this->position_colors_players_.at(player).at(i).at(0) = list_moves.at(n-1).at(0);
            this->position_colors_players_.at(player).at(i).at(1) = list_moves.at(n-1).at(1);
        }
    }
